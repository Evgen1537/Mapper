 ...
 * Project: Mapper
 * Author:  Evgeniy
 * Created: 19-06-2016 23:10
 */
@CommandTemplate(
		id = UIConstants.REFRESH_LAYER,
		text = "Refresh",
		longText = "Refresh tiles from session folder",
		graphic = "/image/arrow-circle-double.png",
		accelerator = "F5",
		path = "/Layer",
		position = 3
)
public class RefreshLayer extends Command {
	...
	private Task<Void> makeTask(@NotNull final List<Long> layerIdList)	{
		...
		return new Task<Void>() {
			...
			protected Void call() throws Exception {
				...
				updateTitle("Refreshing layers...");

				for(Long layerId : layerIdList) {

					layerBean.refreshLayer(
							layerId,
							this::updateMessage,
							this::updateProgress
					);
